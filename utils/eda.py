...
def plot_emotion_counts(
    ...
    if legend_note:
        annotation_dict = format_annotation(annotation_text=legend_note)
        figure.add_annotation(annotation_dict)

    legend_dict = update_legend_format(color_by)

    figure.update_layout(showlegend=True, legend=legend_dict)
...
def update_legend_format(legend_title):
    legend_dict = {
        'title': legend_title.capitalize(),
        'bordercolor': "black",
        'borderwidth': 1  
    }
    return legend_dict

def format_annotation(annotation_text):
    legend_dict = {
